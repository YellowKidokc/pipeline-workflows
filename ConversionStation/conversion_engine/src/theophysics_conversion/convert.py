...
from .converters.whisper_adapter import transcribe_audio_video
...
def load_config(path: Path | None) -> ConversionConfig:
    ...
    flattened = {
        ...
        "whisper_enabled": data.get("whisper", {}).get("enabled", False),
        "whisper_speaker_detection": data.get("whisper", {}).get("speaker_detection", False),
        "whisper_model_size": data.get("whisper", {}).get("model_size", "base"),
        "whisper_language": data.get("whisper", {}).get("language"),
...
def convert(source: str | Path, *, config: ConversionConfig | None = None) -> ConvertResult:
    ...
    if fmt in {Format.SUBTITLE, Format.TRANSCRIPT_JSON, Format.TRANSCRIPT_PANEL}:
        from .converters.transcript_adapter import convert_transcript

        kind = {Format.SUBTITLE: "SUBTITLE", Format.TRANSCRIPT_JSON: "JSON",
                Format.TRANSCRIPT_PANEL: "PANEL"}[fmt]
        result = convert_transcript(path, kind)
        ...
        if config.whisper_enabled:
            result = transcribe_audio_video(
                ...
                speaker_detection=config.whisper_speaker_detection,
            )
        else:
            result = ConvertResult(
                markdown="",
                metadata={"source_format": fmt.value},
                warnings=["Audio/video conversion requires Whisper. Set whisper.enabled: true in the station config."],
            )
    else:
        detail = ""
...
def cli() -> int:
    ...
    if args.out:
        ...
        args.out.with_suffix(".conversion.json").write_text(result.model_dump_json(indent=2), encoding="utf-8")
        print(f"Wrote markdown: {args.out}")
        if result.warnings:
            print(json.dumps({"warnings": result.warnings}, indent=2))
        return 3 if result.metadata.get("review_required") else (0 if result.markdown else 2)
