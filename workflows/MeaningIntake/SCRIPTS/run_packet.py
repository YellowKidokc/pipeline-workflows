...
DEFAULT_CONFIG = {
    ...
    "semantic_provider": "prompt_packet",
...
def process_file(path: Path, run_id: str, config: dict) -> dict:
    ...
    archive = ROOT / "ARCHIVE" / run_id / "sources"
    output = ROOT / "OUTPUT"
    review = ROOT / "REVIEW"
    for directory in (archive, output, review):
        directory.mkdir(parents=True, exist_ok=True)

    source_copy = archive / path.name
    shutil.copy2(path, source_copy)
    base = slug(path.stem)
    record_path = output / f"{base}.knowledge.md"
    receipt_path = output / f"{base}.intake.json"
    prompt_path = review / f"{base}.analysis-prompt.md"
    ...
    prompt_template = (ROOT / "PROMPTS" / "knowledge_record.md").read_text(encoding="utf-8")
    ...
    if confidence < float(config["ask_when_confidence_below"]):
        question_path = review / f"{base}.question.md"
...
def main() -> int:
    ...
    config_path = args.config if args.config.exists() else ROOT / "CONFIG" / "config.example.json"
    if config_path.exists():
        config.update(json.loads(config_path.read_text(encoding="utf-8")))

    for name in ("INPUT", "OUTPUT", "REVIEW", "ARCHIVE", "ERROR", "LOGS"):
    ...
    for path in inputs:
        try:
            results.append(process_file(path, run_id, config))
    ...
    print(f"MeaningIntake processed {len(inputs)} source(s); run {run_id}")
