...
from outbox_layout import EPISTEMIC_RUNS, create_layout
...
def provider_spec(name: str) -> dict[str, str]:
    ...
    raise RuntimeError(f"Unsupported EPISTEMIC_API_PROVIDER: {name}")


# EPISTEMIC_API_PROVIDER accepts a comma-separated preference chain, best route
# first.  The run starts on the first provider and latches forward to the next
# one when a route is exhausted (no key, hard HTTP block, or retries spent).
# Latching is deliberate: once a free route is out of quota for the day, every
# later document in the batch should go straight to the backup instead of
# paying the retry cost again.
PROVIDER_CHAIN = [
    part.strip().lower()
    for part in os.environ.get("EPISTEMIC_API_PROVIDER", "openrouter,deepseek").split(",")
    if part.strip()
] or ["openrouter", "deepseek"]
_SPECS = [provider_spec(name) for name in PROVIDER_CHAIN]
_PROVIDER_INDEX = 0
_PROVIDER_LOCK = threading.Lock()

PROVIDER = _SPECS[0]["provider"]
MODEL = _SPECS[0]["model"]
API_URL = _SPECS[0]["url"]
API_KEY_NAME = _SPECS[0]["key_name"]


def _activate(index: int) -> None:
    """Point the module-level route globals at chain entry `index`."""
    global _PROVIDER_INDEX, PROVIDER, MODEL, API_URL, API_KEY_NAME
    spec = _SPECS[index]
    _PROVIDER_INDEX = index
    PROVIDER = spec["provider"]
    MODEL = spec["model"]
    API_URL = spec["url"]
    API_KEY_NAME = spec["key_name"]


def advance_provider(reason: str) -> bool:
    """Latch to the next route in the chain.  Returns False when none are left."""
    with _PROVIDER_LOCK:
        if _PROVIDER_INDEX + 1 >= len(_SPECS):
            return False
        failed = PROVIDER
        _activate(_PROVIDER_INDEX + 1)
        print(f"ROUTE FAILOVER: {failed} -> {PROVIDER} ({reason})", flush=True)
        return True
...
def call_json(label: str, prompt: str, timeout: int, retries: int) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    while True:
        try:
            data, usage = call_json_on_route(label, prompt, timeout, retries, max_output_tokens)
            # Stamp the route that actually served this call.  A document can
            # straddle a failover, so attribution has to be per call, not per run.
            usage = dict(usage or {})
            usage["_route"] = {"provider": PROVIDER, "model": MODEL}
            return data, usage
        except RouteExhausted as exc:
            if not advance_provider(f"{label}: {exc}"):
                raise RuntimeError(f"{label} failed on every configured route: {exc}") from exc


class RouteExhausted(RuntimeError):
    """The current provider cannot serve this call; try the next one in the chain."""


def call_json_on_route(
    label: str, prompt: str, timeout: int, retries: int, max_output_tokens: int
) -> tuple[dict[str, Any], dict[str, Any]]:
    # The request body is rebuilt per route because the model name and the
    # OpenRouter-only reasoning block differ between providers.
    try:
        headers = api_headers()
    ...
    request_body = json.dumps(
        {
            ...
            **({"reasoning": {"exclude": True, **(
                {"max_tokens": int(os.environ["OPENROUTER_REASONING_MAX_TOKENS"])}
                if int(os.environ.get("OPENROUTER_REASONING_MAX_TOKENS", "0")) > 0 else {}
        ...
        try:
            ...
            try:
                if finish_reason == "length":
                    raise ValueError("Provider truncated the response; incomplete output cannot pass intake")
            ...
            if isinstance(exc, urllib.error.HTTPError):
                ...
                if exc.code in (400, 401, 402, 403, 404):
                    # Auth, credit, and model-availability blocks never clear by
                    # retrying this route -- hand the call to the next provider.
                    raise RouteExhausted(f"blocked: {last_error}") from exc
            else:
                last_error = exc
            print(f"{label} attempt {attempt}/{retries} failed: {last_error}", flush=True)
            if attempt < retries:
                time.sleep(2**attempt)
    raise RouteExhausted(f"failed after {retries} attempts: {last_error}")
...
def call_routes(*usages: dict[str, Any]) -> list[dict[str, str]]:
    """Per-call route attribution, in call order, from the stamped usage blocks."""
    routes = []
    for index, usage in enumerate(usages, start=1):
        route = (usage or {}).get("_route") or {}
        routes.append({
            "call": f"call_{index}",
...
def distinct_field(routes: list[dict[str, str]], field: str) -> str:
    """One value when every call agreed, otherwise a mixed list showing the split."""
    seen = [route[field] for route in routes if route.get(field) and route[field] != "unknown"]
...
def render_markdown(source_path: Path, source_hash: str, text: str, extraction: dict[str, Any], evaluation: dict[str, Any], synthesis: dict[str, Any], run_id: str, routes: list[dict[str, str]] | None = None) -> str:
    routes = routes or []
    route_provider = distinct_field(routes, "provider") if routes else PROVIDER
    route_model = distinct_field(routes, "model") if routes else MODEL
    routes_yaml = "[" + ", ".join(
        f'"{route["call"]}={route["provider"]}/{route["model"]}"' for route in routes
    ) + "]" if routes else "[]"
...
run_id: {run_id}
provider: {yaml_value(route_provider)}
model: {yaml_value(route_model)}
call_routes: {routes_yaml}
...
- **Run ID:** `{run_id}`
- **Intake Engine:** Epistemic Intake v2 (`{route_model}`)
...
def process_one(path: Path, run_id: str, timeout: int, retries: int, dry_run: bool) -> dict[str, Any]:
    ...
    # --input may point outside INBOX (e.g. WAITING_TO_BE_PROCESSED). relative_to
    # raises ValueError in that case, and it raised HERE - after all three API
    # calls had already completed - so paid-for work was discarded at the filing
    # step. Fall back to the source's own parent folder name instead.
    try:
        _rel_parent = path.relative_to(INBOX).parent
    except ValueError:
        _rel_parent = Path(path.parent.name)
    processed_dir = PROCESSED / run_id / _rel_parent
    create_layout()
    ...
    routes = call_routes(usage1, usage2, usage3)
    markdown = render_markdown(path, digest, text, extraction, evaluation, synthesis, run_id, routes)
    ...
    receipt = {
        ...
        "routed_outputs": routed_paths,
        "provider": distinct_field(routes, "provider"),
        "model": distinct_field(routes, "model"),
        "call_routes": routes,
...
def main() -> int:
    ...
    if args.check:
        ...
        print(f"PROVIDER_CHAIN={','.join(PROVIDER_CHAIN)}")
        for index, spec in enumerate(_SPECS):
            role = "primary" if index == 0 else f"backup {index}"
            configured = "yes" if env_or_registry(spec["key_name"]) else "no"
            print(f"  [{role}] {spec['provider']} model={spec['model']} key_configured={configured}")
        # Probe each route independently so a healthy backup is visible even
        # when the primary is the one that is down.
        ready = []
        for index, spec in enumerate(_SPECS):
            _activate(index)
            try:
                probe, _ = call_json_on_route("Preflight", 'Return exactly {"connected":true}.', 60, 1, 1024)
                if probe.get("connected") is not True:
                    raise RuntimeError("unexpected preflight response")
                print(f"ROUTE[{spec['provider']}]=ready")
                ready.append(spec["provider"])
            except Exception as exc:
                print(f"ROUTE[{spec['provider']}]=blocked ({exc})")
        _activate(0)
        if ready:
            print(f"API_ROUTE=ready (usable: {', '.join(ready)})")
            return 0
        print("API_ROUTE=blocked (no usable route in the chain)")
        return 1
