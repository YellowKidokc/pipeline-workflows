...
def process_file(path: Path, run_id: str, config: dict, known_hashes: dict[str, dict]) -> dict:
    ...
    from outbox_layout import MASTER
    ready = MASTER
...
def main() -> int:
    ...
    for name in (
        "INBOX", "PROCESS", "OUTBOX/00_PRESERVED_RECORDS",
        "OUTBOX/02_EVIDENCE_AND_AUDITS/03_READY_TO_TRANSFER",
        "OUTBOX/02_EVIDENCE_AND_AUDITS/04_TRANSFERRED",
        "OUTBOX/99_HOLD_REVIEW", "QUESTIONS", "ORIGINALS", "TRANSFER", "LOGS"
    ):
