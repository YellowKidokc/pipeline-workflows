#!/usr/bin/env python3
"""AXIOM_NODES station runner — map papers to canonical axiom nodes.

Reads WORKSPACE/01_PRIORITY, WORKSPACE/02_SERIES, WORKSPACE/03_GENERAL (.md/.txt),
calls the LLM, appends axiom-node mapping to the bottom of each paper,
and writes JSON records to WORKSPACE/06_JSON_RECORDS.
...
from axiom_io import paper_uuid, write_json_record, enrich_markdown_companion, preserve_original
...
def _process_item(workspace: Path, item: Path, provider: str) -> dict:
    ...
    print(f"[{short_uuid}] {item.name} — mapping axiom nodes", flush=True)
    prompt = build_axiom_prompt(source_text)
    result = complete(provider, prompt, json_mode=True)

    try:
        data = _parse_json(result)
    except json.JSONDecodeError:
        # The answer was likely cut off. Ask once more for a compact, complete response.
        print(f"[{short_uuid}] {item.name} — JSON truncated, retrying compact", flush=True)
        retry_prompt = (
            prompt
            + "\n\nIMPORTANT: your previous answer was too long and was cut off. "
            "Return the same JSON structure, complete and valid, but much more compact: "
            "only the most relevant axiom nodes (at most 10-15), shorter strings, no long quotes."
        )
        result = complete(provider, retry_prompt, json_mode=True)
        data = _parse_json(result)
    ...
    data.setdefault("primary_mode", "UNKNOWN")
    data.setdefault("axiom_nodes", [])
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="AXIOM_NODES station runner")
    parser.add_argument(
        "--root",
        type=Path,
        default=None,
        help="AXIOM_NODES station root directory (default: parent of script)",
    )
    parser.add_argument(
        "--workspace",
        type=Path,
    ...
    print(f"AXIOM_NODES station: {len(items)} eligible item(s) in {workspace}")
