...
REASONING_OPTIONS: dict[str, dict[str, Any]] = {}
...
def _run_probe_once(model_id: str, probe: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "temperature": 0,
        "max_tokens": 4096,
        "reasoning": REASONING_OPTIONS.get(model_id, {"exclude": True}),
    ...
    try:
        ...
        if finish == "length":
            raise ValueError("Probe response truncated at its output token limit")
        if not content:
            raise ValueError(f"Probe returned empty content; finish_reason={finish}")
...
def run_probe(model_id: str, probe: dict[str, Any]) -> dict[str, Any]:
    # Bound retries and pace even failed/denied requests on the free route.
    result = {}
    for attempt in range(1, 3):
        time.sleep(3.2)
        result = _run_probe_once(model_id, probe)
        result["requests_used"] = attempt
        print(f"  {probe['name']}: {'PASS' if result['passed'] else 'FAIL'}"
              f" ({result['latency_seconds']}s)", flush=True)
        if result.get("error"):
            print(f"    {result['error']}", flush=True)
        if result["passed"] or any(f"HTTP {code}" in str(result.get("error"))
                                   for code in (400, 401, 402, 403, 404, 429)):
            break
    return result
...
def main() -> int:
    ...
    for model in models:
        ...
        if (model.get("reasoning") or {}).get("supports_max_tokens"):
            options["max_tokens"] = 2048
        REASONING_OPTIONS[str(model["id"])] = options
    if not models:
    ...
    for model in eligible:
        ...
        try:
            recently_passed = prior.get("status") == "PASS" and prior.get("reasoning_options") == REASONING_OPTIONS.get(model_id) and datetime.fromisoformat(prior_time) >= cutoff
        except (TypeError, ValueError):
            pass
        if recently_passed:
            chosen = model
        ...
        if len([x for x in tested if x.get("status") != "REUSED_PASS"]) >= max(1, args.candidate_count):
            continue
        print(f"Testing free model: {model_id}", flush=True)
        probes = []
        for probe in PROBES:
            result = run_probe(model_id, probe)
        ...
        record = {
            ...
            "reasoning_options": REASONING_OPTIONS.get(model_id),
        }
    ...
    if chosen is None:
        raise RuntimeError(f"No tested free model passed today's deterministic gate. Details: {HISTORY_PATH}")

    chosen_id = str(chosen["id"])
    probe_requests = sum(probe.get("requests_used", 1) for record in tested for probe in record.get("probes", []))
    ...
    plan = {
        ...
        "reasoning_options": REASONING_OPTIONS.get(chosen_id),
    ...
    with CMD_PATH.open("a", encoding="utf-8") as route:
        budget = REASONING_OPTIONS.get(chosen_id, {}).get("max_tokens", 0)
        route.write(f'set "OPENROUTER_REASONING_MAX_TOKENS={budget}"\n')
