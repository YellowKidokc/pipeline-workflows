...
class ObsidianDefinitionsManager:
    ...
    def _load_config(self) -> None:
        ...
        self.definitions_folder = self._resolve_definitions_folder(def_folder_str)

    def _resolve_definitions_folder(self, configured_folder: str) -> Optional[Path]:
        """Resolve definitions folder using configured value + vault-aware fallbacks."""
        candidates: List[Path] = []

        if configured_folder:
            configured = Path(configured_folder)
            if configured.is_absolute():
                candidates.append(configured)
            elif self.vault_path:
                candidates.append(self.vault_path / configured)

        if self.vault_path:
            candidates.extend([
                self.vault_path / "00_SYSTEM" / "Glossary",
                self.vault_path / "02_LIBRARY" / "Glossary",
                self.vault_path / "02_LIBRARY" / "4_LEXICON",
                self.vault_path / "99_MATH_APPENDIX" / "Definitions",
                self.vault_path / "00_AXIOMS",
            ])
        ...
        # Return a sensible writable path even if nothing exists yet.
        if configured_folder:
            configured = Path(configured_folder)
            if configured.is_absolute():
                return configured
            if self.vault_path:
                return self.vault_path / configured
        if self.vault_path:
            return self.vault_path / "00_SYSTEM" / "Glossary"
        return None
    ...
    def set_vault_path(self, vault_path: Path) -> None:
        ...
        self.definitions_folder = self._resolve_definitions_folder(def_folder)
    ...
    def scan_definitions(self) -> List[DefinitionFile]:
        ...
        if not self.definitions_folder or not self.definitions_folder.exists():
            self.definitions_folder = self._resolve_definitions_folder(
