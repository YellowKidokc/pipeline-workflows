...
def save_output(text: str, model: str, source_file: pathlib.Path = None, output_path: pathlib.Path = None) -> pathlib.Path:
    """Write the response alongside the source file, or to output/ as fallback."""
    if output_path and source_file:
        # Write to the configured output path, named after the source file
        output_path.mkdir(parents=True, exist_ok=True)
        stem = source_file.stem.replace("_CLEAN", "")
        out_path = output_path / f"{stem}_TAGGED.md"
    elif source_file:
        # Write alongside the source file
        stem = source_file.stem.replace("_CLEAN", "")
        out_path = source_file.parent / f"{stem}_TAGGED.md"
    else:
...
def main():
    ...
    output_path = pathlib.Path(output_path_str) if output_path_str else None
    ...
    for rel_name, abs_path, kind in files:
        if kind == "binary_skip":
            print(f"\n[SKIP] {rel_name} (binary)")
            continue

        print(f"\nProcessing: {rel_name}")
        print("-" * 40)

        single_file_messages = build_messages(prompt_text, [(rel_name, abs_path, kind)])
        ...
        kwargs = {
            "model": model,
            "messages": single_file_messages,
        }
        if not model.startswith("o"):
            kwargs["temperature"] = temperature
        if max_tokens:
            # Reasoning models (o-series) expect max_completion_tokens.
        ...
        # Save output alongside source (or to OUTPUT_PATH if configured)
        out_path = save_output(reply, model, source_file=abs_path, output_path=output_path)
    ...
    print(f"  Total cost:")
    print(format_cost(model, total_in_tok, total_out_tok))
    print("=" * 60)
