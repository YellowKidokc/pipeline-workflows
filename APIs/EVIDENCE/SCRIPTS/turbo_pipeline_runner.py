...
def call_deepseek_raw(
    ...
    if deepseek_key:
        deepseek_direct = {
            "provider": "deepseek",
            "model": preferred_model or os.environ.get("DEEPSEEK_MODEL", "deepseek-flash"),
...
class PaperProcessor:
    ...
    def process_single_paper(self, paper_path: Path, is_priority: bool, series_name: str | None, force: bool = False) -> dict:
        ...
        if not force and self._is_already_completed(file_sha, slug):
            ...
            dest_orig = PROCESSED_DIR / f"{slug}_{uuid.uuid4().hex[:12]}{paper_path.suffix}"
...
"""
        ...
        # Companion on top, ORIGINAL ARTICLE unchanged below.
        for _out in (c1_file, c2_file, c4_file, substack_file):
            article_stack.write_stacked(_out, rendered_output, original_bytes)

        if out_series:
            c3_file = out_series / f"{slug}_C3_{paper_uuid}.md"
            article_stack.write_stacked(c3_file, rendered_output, original_bytes)
        ...
        record_output(dest_orig, rendered_output, status="processed_original")
...
def collect_workload() -> list[tuple[Path, bool, str | None]]:
    ...
    if not INBOX_DIR.exists():
        return queue
    skip = {'.git', '.lake', '.venv', '__pycache__', 'node_modules'}
    for directory, dirs, names in os.walk(INBOX_DIR):
        dirs[:] = sorted(d for d in dirs if d not in skip)
        for name in sorted(names):
            f = Path(directory) / name
            if f.suffix.lower() not in ALLOWED_EXTS:
                continue
            parts = f.relative_to(INBOX_DIR).parts
            priority = parts[0] == '_PRIORITY'
            folders = list(parts[:-1])
            if folders and folders[0] == '_PRIORITY': folders.pop(0)
            if folders and folders[0] == 'SERIES': folders.pop(0)
            series = folders[0] if folders else None
            queue.append((f, priority, series))
    return sorted(queue, key=lambda row: (not row[1], str(row[0])))
...
def main():
    ...
    parser.add_argument("--single-batch", action="store_true", help="Process the selected snapshot once")
    ...
    for directory in (INBOX_DIR, OUTBOX_DIR, FAILED_DIR, LOGS_DIR, PROCESSED_DIR):
    ...
    session = Session(ROOT_DIR, args.provider, args.model, args.workers)
    total_completed = 0
    total_errors = 0
    ...
    while True:
        ...
        if batch_total == 0:
            if args.continuous:
                print("[Watcher] Inbox is empty. Waiting for documents (Ctrl+C to stop)...", end="\r")
                time.sleep(2)
                continue
            else:
                if total_completed > 0 or total_errors > 0:
                    print(f"\nAll inbox documents processed: {total_completed} Succeeded, {total_errors} Failed.")
                else:
                    print("Inbox is empty. Drop files into INBOX or INBOX/_PRIORITY.")
                break

        print(f"\nFound {batch_total} document(s) in intake queue. Processing with {args.workers} workers...")

        session.select(workload)
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            ...
            for future in as_completed(future_map):
                ...
                try:
                    res = future.result()
                    session.mark(p, res.get("status", "SUCCESS"))
                    total_completed += 1
                    prio_tag = "[PRIORITY]" if res["is_priority"] else ""
                    series_tag = f"[{res['series']}]" if res["series"] else ""
                    print(f"[{total_completed}] [OK] SUCCESS: {res['slug']} {prio_tag}{series_tag} | Standing: +{res['score']}/8.0 in {res['elapsed_seconds']}s")
                except Exception as e:
                    session.mark(p, "FAILED", str(e))
                    total_errors += 1
                    print(f"[ERROR] Failed processing {p.name}: {e}")
                    FAILED_DIR.mkdir(parents=True, exist_ok=True)
                    dest_err = FAILED_DIR / f"ERR_{uuid.uuid4().hex[:12]}_{p.name}"
    ...
    print("\n" + "="*50)
    print(f"INTAKE COMPLETE: {total_completed} Succeeded, {total_errors} Failed.")
