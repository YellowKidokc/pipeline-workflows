...
class ChiEvaluation:
    ...
    def geo_mean(self) -> float:
        """Geometric mean of channel scores — intuitive per-channel average (0–1)."""
        scores = [c.effective_score for c in self.channel_results]
        if not scores:
            return 0.0
        product_val = chi_product(scores)
        if product_val <= 0:
            return 0.0
        return round(product_val ** (1.0 / len(scores)), 6)
...
def _extract_channels(d: dict) -> list[dict]:
    """
    Handle three JSON shapes models return:
    1. Full:     {"channel_results": [{"channel": "G", ...}]}
    2. O3 flat:  {"G": {...}, "M": {...}, ...}
    3. DeepSeek: {"channels": {"G": {...}, "M": {...}}}
    """
    # Shape 1 — preferred
    if "channel_results" in d and isinstance(d["channel_results"], list):
        return d["channel_results"]

    # Shape 3 — channels sub-object
    if "channels" in d and isinstance(d["channels"], dict):
        src = d["channels"]
        return [_normalize_channel_dict(k, v) for k, v in src.items()
