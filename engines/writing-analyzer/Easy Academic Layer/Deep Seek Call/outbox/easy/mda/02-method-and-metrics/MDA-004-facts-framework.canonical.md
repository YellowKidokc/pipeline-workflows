...
---
claims:
  - "American society shows measurable coherence decay across 23 independent domains, all following the same mathematical pattern with inflection points clustered between 1958 and 1968."
  - "The decay curves match the Lindbladian dissipation equation from quantum mechanics, suggesting social systems obey the same coherence dynamics as physical systems."
  - "Cross-domain statistical tests reject the null hypothesis of independent decay (K-S test p = 0.003), meaning the synchronization is far too unlikely to be random."
  - "The visible ruptures of 1968–1973 represent the completion of a phase transition, not its initiation — the decay began a decade earlier."
  - "Coherence requires constraint: the systematic removal of constraints across legal, economic, cultural, and technological domains between 1958-1973 triggered the synchronized collapse."
  - "No existing single-domain theory (economic, cultural, political) accounts for cross-domain synchronization — the answer requires cross-domain coherence analysis."
  - "The paper can be falsified by five specific kill conditions, including finding a major domain showing increasing coherence without external constraint intervention."
domains:
  Physics: 20
  Theology: 15
  Mathematics: 15
...
This paper shows a striking mathematical pattern: many different parts of American life — including family structure, religious affiliation, trust in institutions, mental health, and economic stability — all show nearly identical decline curves. The turning points all cluster in the **1958–1968** window. The visible breakdowns of 1968–1973 were the _completion_ of a phase transition, not its beginning. Cross-domain statistical tests show the chance of this happening randomly is very low (K-S test p = 0.003).

The decline follows exponential decay:

$$\chi(t) = \chi_0 \cdot e^{-\lambda t} + C$$

This means the system loses coherence at a steady rate over time, like a spinning top gradually slowing down.

The average decay constant is **λ = 0.045 ± 0.050** and the median is **λ = 0.023**. This mathematical form matches the equations that describe loss of quantum coherence in open systems.

**This pattern is far too unlikely to be random — it demands explanation.**

We do not claim to know what causes it. Instead, this paper documents the pattern, measures the correlation, and proposes a single tracking metric — the Coherence Index (χ) — to monitor systemic health across different domains. All statistical claims can be reproduced using the computation pipeline (moral_decay_compute.py).

## 1. Introduction

Before presenting evidence, I am required by my own methodology to tell you my bias.
...
| Core Belief | Reality is structured by a coherent Logos (divine reason); disorder is deviation from design |
| Epistemology | Empirical observation constrained by logical coherence and falsifiability (the ability to be proven wrong) |
| Priors | I began this research believing America was in decline. The data could have refuted this. It did not. |
| Off-Ramp | Reasonable people may interpret identical curves as coincidence or artifact. I cannot eliminate this possibility — only make it statistically improbable. |
...
1. Twenty-three domains (of 45 surveyed) show statistically significant decline beginning 1958–1968
...
At least one major domain will show increasing coherence during the study period without external constraint intervention. The curves will not survive replication with alternative datasets.

## 3. Literature Review

### The Disciplinary Blind Spot

Each academic discipline has documented decline within its own area:
...
Each domain was converted to a 0–1 scale where 1.0 = peak coherence and 0.0 = theoretical minimum.

$$\chi_i(t) = \frac{X_i(t) - X_{i,\min}}{X_{i,\max} - X_{i,\min}}$$

This formula simply rescales each dataset so they can be compared directly, like converting different currencies to a single dollar value.
...
Where χ₀ = initial coherence, λ = decay constant, t₀ = inflection point, A, ω, φ = perturbation parameters, C = asymptotic floor. The sine term accounts for small wobbles in the data, like waves on a receding tide.
...
**Critical Reframing:** The mean inflection point of **1958.6** indicates that coherence decay _initiated_ in the late 1950s — earlier than previously assumed. The 1968–1973 period represents the _completion_ of the phase transition, not its beginning.
...
**Key Finding:** The t₀ estimate is very robust — all analyses converge on 1954–1962. The λ estimate shows moderate sensitivity, warranting the wider confidence interval reported above.
...
Think of it like a tuning fork — when you strike it near other objects, they start vibrating at the same frequency. One source of perfect pitch that brings everything else into harmony. But when the tuning fork is exposed to too much noise, it stops vibrating cleanly. The system loses its coherence.
...
| Period | Phase |
|---|---|
| 1945–58 | Peak coherence (post-war consensus) |
| 1958–63 | Inflection initiation (invisible decay begins) |
| 1968–73 | Phase transition (visible rupture) |
| 1973–now | Post-transition decay (new trajectory) |
...
Think of it like the slope of a hill — you don't get pushed, the ground itself tilts so you naturally move toward the bottom. Grace changes the shape of the ground under your feet. When constraints are removed, the slope changes, and the system naturally moves toward disorder.
