...
---
claims:
  - "The lifespan data in Genesis chapters 5 and 11, when plotted, fits an exponential decay curve with high precision (R² = 0.888), proving it is a real biological dataset, not mythology."
  - "The model independently finds a lifespan floor of 93 years, which matches Psalm 90:10's 70-80 years and the modern global average of 73 years, showing three independent measurements of the same physical reality."
  - "The exponential decay is caused by 'generational entropy' — a compounding degradation of the information channel between humans and the Logos (God's design), introduced at the Fall in Genesis 3."
  - "The lifespan floor is held by 'common grace,' which acts like the χ vacuum field in physics — a minimum coherence that thermodynamics cannot breach, preventing human lifespan from decaying to zero."
  - "The Flood reset some of this accumulated entropy, but not to zero, which is why the post-Flood curve restarts from a lower baseline (Shem at 600 years) rather than continuing from the pre-Flood plateau."
  - "Psalm 90 is not a cultural observation but a precise statement about this asymptotic floor, written by Moses 3,000 years before modern math could model it."
domains:
  Physics: 25
  Theology: 30
  Mathematics: 15
...
# The 93-Year Floor: What the Genesis Genealogies Are Actually Measuring
...
This equation means: the lifespan at any generation (L(n)) equals the starting lifespan (L₀) multiplied by a decay factor (e^{-n/τ}), plus a minimum floor value (L_floor). The decay factor shrinks as generations pass.
...
| Measurement | Value |
|---|---|
| Decay Model | 93 years |
| Moses (Psalm 90) | 70–80 years |
| Modern Average | 73 years |
...
Entropy accumulates this way. The framework calls it *generational entropy* — the accumulated misalignment with the Logos (the original design and information of God) that compounds across generations, each generation inheriting the decoherence (loss of alignment) of all prior generations plus their own. The first generation after the Fall has Adam's entropy. The second has Adam's plus Seth's. And so on. The compounding is the mechanism. The exponential is the signature.
...
Not because of moral failure alone, but because the information channel between the human system and the Logos undergoes signal degradation at each generational remove. Think of it like a radio signal through static — the clearer the channel, the more of the message gets through. Sin is the static. Truth is the signal.
...
- **Exponential** because each If is a fixed fraction of the last
