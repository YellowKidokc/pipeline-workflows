...
claims:
  ...
  - "The logistic equation dPhi/dt = sigma * gamma * (Phi_max - Phi) * Phi mathematically models moral trajectory evolution, producing two stable attractors and one unstable saddle point at Phi=0.5."
  ...
  - "Grace acts as a basin-shifting force that can move a trajectory from the Hell basin (Phi<0.5) to the Heaven basin (Phi>0.5), which is impossible through self-effort alone."
  - "Population-level moral coherence distributions should show two peaks (bimodal), not a bell curve — this is a testable prediction."
  - "Moral trajectories exhibit critical slowing near the saddle point, meaning people near the decision boundary experience prolonged indecision before committing to one attractor."
  - "The model is structurally sound but the binary outcome is imposed by the chosen equation, not derived from deeper principles — it requires theological acceptance of bimodal destiny."
domains:
  Physics: 30
  Theology: 30
  Mathematics: 20
...
**Concept A:** A dynamical system (a system that changes over time according to fixed rules) with two stable fixed-point attractors (final resting places) and one unstable saddle point (a tipping point). The moral state Phi evolves under logistic-type dynamics (a growth equation that levels off), producing a bimodal phase portrait (a map showing two possible destinations) with basins of attraction (regions that pull you toward one destination) centered on Phi = 0 and Phi = 1. No neutral terminus exists — all trajectories (paths) terminate at one of the two attractors.

**Concept B:** Heaven as maximum coherence (perfect alignment with God, Phi -> 1). Hell as minimum coherence (complete separation from God, Phi -> 0). No neutral final state — every person's trajectory ends in one of two destinations. The "narrow gate" (Matthew 7:13-14) corresponds to the unstable saddle at Phi = 0.5 that separates the two basins.
...
This equation says: The rate of change of your moral state over time equals your moral sensitivity times the grace coefficient times the gap between your maximum potential and your current state, times your current state.

Where:
- Phi in [0, 1] is the moral/coherence state (0 = no coherence, 1 = perfect coherence)
- sigma = moral sensitivity (how responsive you are to input)
- gamma = grace coefficient (how much grace is available)
- Phi_max ~ 1.0 = maximum moral potential

**Phase portrait analysis** (a map of all possible paths):

- **Fixed point Phi = 0:** Stable attractor (Hell). dPhi/dt = 0, and perturbations (small pushes) bring you back toward 0. Like a marble at the bottom of a bowl — if you nudge it, it rolls back down.
- **Fixed point Phi = 1:** Stable attractor (Heaven). dPhi/dt = 0, and perturbations bring you back toward 1. Like a marble at the bottom of a different bowl.
- **Fixed point Phi = 0.5:** Unstable saddle. Any perturbation pushes the trajectory toward one of the two stable attractors. Like a marble balanced on the ridge between two bowls — the slightest breeze sends it rolling into one.

**Basin of attraction** (which bowl you're in):
...
This says: You reach Heaven if the total accumulated effect of your moral sensitivity times your rate of change over time exceeds a certain threshold.
...
- The system should exhibit hysteresis (a lag effect): once a trajectory is deep in a basin, it requires a larger perturbation to escape than it would near the saddle. This maps to the theological intuition that late conversion is harder than early conversion (but not impossible with sufficient grace).
...
**Falsification** (ways to prove this wrong):
