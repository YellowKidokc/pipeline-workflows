...
---
claims:
  - "Grace reception follows a sigmoidal dose-response curve governed by the Hill equation, just like drugs binding to receptors — below a threshold, no transformation happens; above it, rapid change occurs."
  - "Sin acts as a competitive antagonist that blocks faith-receptors, shifting the grace-response curve to the right so more grace exposure is needed to produce the same effect — but sin can never permanently block grace if grace concentration is sufficient."
  - "The Hill coefficient n represents community faith density: higher n (gathered believers) produces sharper, more dramatic conversion events; isolated faith (n=1) produces gradual change."
  - "Repeated exposure to grace without response causes receptor downregulation (tolerance), matching Hebrews 6:4-6 — but this is reversible through 'drug holidays' (spiritual deserts followed by renewed engagement)."
  - "The pharmacological model predicts that conversion events should cluster bimodally — few people are 'half converted' — and that community faith density should correlate with conversion threshold steepness, both empirically testable."
  - "Grace has no 'excretion' phase — unlike drugs, grace is retained permanently, mapping to the irreversibility of justification in Reformed theology and the indelible character of sacraments in Catholic theology."
  - "The mapping is falsifiable: if grace reception shows no correlation with means of grace, or if sin doesn't impede grace, or if conversion is linear rather than threshold-based, the model collapses."
domains:
  Pharmacology: 30
  Theology: 35
  Mathematics: 15
...
# ISO-033: How Grace Works Like a Drug (Pharmacology of Grace)
...
**Domain A:** Pharmacology — How drugs work in the body: dose-response, receptor binding, how the body processes drugs (ADME), how drugs compete, tolerance, and how receptors change over time.

**Domain B:** Christian Theology — How grace works: prevenient, justifying, and sanctifying grace; means of grace; sacraments; faith as the way we receive grace; sin as what blocks it.

**Concept A:** A drug (called a ligand) binds to a receptor with a certain strength (K_d). The effect follows a curve called the Hill equation. There's a "therapeutic window" — the range between the smallest dose that works and the dose that becomes toxic. Some drugs activate (agonists), some block (antagonists). The system can develop tolerance, sensitivity, and receptors can increase or decrease in number.

**Concept B:** Grace (the divine ligand) connects with humans through faith (the receptor). The response follows a threshold pattern: below the threshold, nothing visible happens. Above the threshold, rapid change. Past saturation, more grace doesn't produce more effect because the receptor system is finite. Sin acts like a competitive antagonist — it occupies the binding site.
...
This equation says: The effect (E) depends on the drug concentration [D]. E_max is the biggest effect possible. EC_50 is the concentration that gives half the maximum effect. n is the Hill coefficient — it controls how steep the curve is.

Receptor binding kinetics:

[D] + [R] ⇌ [DR] K_d = [D][R]/[DR]

This means: Drug [D] and receptor [R] bind together to form a drug-receptor complex [DR]. K_d measures how tightly they bind.

Competitive inhibition by antagonist [A]:

E = E_max x [D]^n / ((EC_50(1 + [A]/K_i))^n + [D]^n)

This means: When an antagonist [A] is present, you need more drug [D] to get the same effect. The antagonist shifts the curve to the right.

**Mathematical Form B:**

Now we swap in theological variables:
...
- alpha = E_max = maximum openness to grace (determined by how much autonomy you have, from ISO-002)
...
- GT_50 = grace threshold for 50% transformation (the "tipping point" — different for each person)
- n = faith-receptor density (how cooperatively the receptors respond; higher n = sharper threshold)
...
1. **Sigmoidal threshold response** — The response isn't linear. Below threshold, nothing visible happens. At threshold, rapid transition. Above saturation, diminishing returns. The Hill equation governs both.
...
3. **The therapeutic window** — Too little = no effect. The "right amount" isn't about dose (grace is unlimited) but about receptor availability and coupling. The narrow way (Matthew 7:14) is a receptor-availability constraint, not a grace-supply constraint.
