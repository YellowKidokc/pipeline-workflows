...
claims:
  ...
  - "Pelagianism (self-salvation) is structurally incoherent because it violates the same logic that makes perpetual motion machines impossible."
  - "The coupling coefficient α(u) shows that a fully closed system (u=1) receives zero grace, while a fully open system (u=0) receives maximum coherence restoration."
  - "Any coherent rescue system must have all five grace properties; missing any one creates the specific failure pattern of the corresponding theorem."
  - "The mathematics constrains which theological rescue systems are structurally possible, and the theology describes a rescue with exactly the required properties."
  - "Falsification requires breaking any one of the six theorems — none have been broken in the history of mathematics or physics."
domains:
  Mathematics: 25
  Physics: 20
  Theology: 30
...
1. **Gödel's Incompleteness Theorems** — No powerful enough math system can prove it's not lying to itself from the inside. You can't check your own homework.
...
3. **Turing's Halting Problem** — No computer program can predict whether it will ever finish running. Self-prediction is impossible.
4. **Clausius / Second Law of Thermodynamics** — Like heat flowing from hot to cold — it only moves one direction, and you can't make it go backwards without spending energy from outside the system. Entropy (disorder) in a closed system never decreases on its own.
5. **Landauer's Principle** — Every time a computer erases information, it has to release heat. Computation isn't free — it always costs energy.
6. **Leibniz's Contingency Argument** — Nothing that could have been different explains why it exists at all. You need something that couldn't have been different to ground everything else.
