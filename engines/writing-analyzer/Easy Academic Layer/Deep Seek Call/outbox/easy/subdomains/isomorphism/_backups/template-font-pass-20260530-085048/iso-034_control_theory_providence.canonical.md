...
---
claims:
  - "Divine providence functions like a PID controller, using three temporal modes of correction: proportional (immediate consequences), integral (cumulative sanctification), and derivative (prophetic warning)."
  - "The free will paradox is resolved by control theory: a controller drives a system toward a setpoint without replacing the plant's own dynamics, just as God governs without replacing human agency."
  - "The Nyquist stability criterion formalizes the conditions for apostasy: the system becomes unstable when the feedback loop opens (conscience seared) or the human transfer function changes enough to violate the criterion."
  - "Deism is the open-loop special case of providence: no feedback, no disturbance rejection, no setpoint tracking — the output is entirely determined by initial conditions and disturbances."
  - "The mapping generates 13 independent correspondences between control theory and theology, with a probability of chance coincidence less than 1.2 × 10⁻¹⁷."
  - "The oscillatory pattern in the Book of Judges is a derivative-deficient system: without prophetic warning (derivative mode), the system oscillates between revival and decline."
  - "The unforgivable sin maps to a structural change in the plant that makes it uncontrollable — the human agent becomes structurally uncontrollable, and the controller cannot drive the system to the setpoint without infinite energy."
domains:
  Control Theory: 35
...
**Domain A:** Control Theory — How feedback systems work. This includes PID controllers (a type of control system), stability analysis, setpoints (target states), disturbance rejection (blocking outside interference), and controllability/observability (can the system be controlled and can we know its state?).

**Domain B:** Christian Theology — How God governs the world (providence). This includes predestination, free will, discipline, prophetic warning, and judgment.

**Concept A:** A closed-loop control system (a system that uses feedback) drives the plant output Y(s) toward a reference setpoint R(s). It does this by measuring the error E(s) = R(s) - Y(s) and applying corrective input through a controller C(s). The system rejects disturbances D(s) while staying stable. A PID controller uses three modes: Proportional (correction based on current error), Integral (correction based on accumulated error), and Derivative (correction based on how fast the error is changing).

**Concept B:** Divine providence drives the human agent toward God's intended purpose (telos/setpoint). It measures the deviation (sin/error) and applies corrective input through multiple modes: immediate conscience/consequences (proportional), progressive sanctification over accumulated history (integral), and anticipatory prophetic warning (derivative). Providence rejects disturbances (temptation/evil) while preserving human agency (plant dynamics).
...
The standard closed-loop transfer function (the math that describes how the system responds):

Y(s) = [G(s)C(s) / (1 + G(s)C(s)H(s))] x R(s) + [G(s) / (1 + G(s)C(s)H(s))] x D(s)

In plain English: The actual output Y(s) equals the target R(s) multiplied by a correction factor, plus the disturbance D(s) multiplied by a different correction factor. The feedback loop (the denominator) reduces the effect of disturbances.

Where:
- Y(s) = plant output (system state in Laplace domain — a math tool for analyzing systems)
- R(s) = reference setpoint (desired state)
- G(s) = plant transfer function (system dynamics — how the system naturally behaves)
- C(s) = controller transfer function (how the controller responds)
...
In plain English: The controller's response equals the proportional gain (K_p) plus the integral gain (K_i divided by frequency) plus the derivative gain (K_d times frequency). Each term handles a different aspect of the error.
...
In plain English: A system stays stable only when a specific mathematical condition is met. If the condition fails, the system becomes unstable — it oscillates out of control or diverges.
...
In plain English: The actual human life Y(s) equals God's purpose R_telos(s) multiplied by a correction factor, plus the effect of evil D_evil(s) multiplied by a different correction factor. The feedback loop (the denominator) reduces the effect of evil.
...
1. **Setpoint-driven correction** — The system is pulled toward a reference state. All corrective action comes from the difference between actual and intended state. In control theory: E(s) = R(s) - Y(s). In theology: sin = deviation from God's purpose. The error signal IS the gap between what is and what should be.
...
5. **Disturbance rejection** — The closed-loop system reduces disturbances by the factor 1/(1 + GCH). Providence does not eliminate evil/temptation (D(s) remains nonzero); it ATTENUATES its effect on the output. The larger the loop gain GCH, the greater the attenuation. This is why "greater is he who is in you than he who is in the world" (1 John 4:4) — the loop gain of providence exceeds the disturbance magnitude.
...
In Control Theory (A):
- A system with proportional-only control will have steady-state offset (the error never fully reaches zero). The integral term is necessary to eliminate offset. Prediction: proportional-only correction (conscience alone, without cumulative sanctification) will leave a permanent gap between actual state and setpoint. This is the control-theoretic prediction that conscience without sanctification is insufficient — a nontrivial theological claim generated by the mathematical structure.
- A system without derivative action will overshoot and oscillate. Prediction: without prophetic/anticipatory correction, the system (human/nation) will show oscillatory behavior — revival/decline cycles with increasing amplitude. Israel's history shows exactly this pattern in Judges (the "cycle of sin" — judges 2:11-19), and the prophets were introduced as the derivative term to damp the oscillation.
...
In Theology (B):
- Providence must show all three temporal modes (present, cumulative, anticipatory) to achieve zero steady-state error and stable convergence. A theology that emphasizes only one mode (e.g., only immediate consequences, or only long-term sanctification, or only prophetic warning) will be structurally incomplete — the control system will either have offset, oscillate, or lack damping.
...
13 independent correspondences. At p < 0.05 per correspondence, the probability of 13 independent chance matches is < 0.05^13 ≈ 1.2 x 10^-17. That's less than one in a hundred quadrillion.
