...
---
claims:
  - "Six independent theorems from mathematics and physics prove that closed systems cannot self-validate, define their own truth, predict their own termination, reverse their own entropy, compute without external energy cost, or explain their own existence."
  - "Grace, as described in Ephesians 2:8-9, has exactly five properties (externality, sufficiency, asymmetric cost, merit-independence, temporal priority) that are the logical inversions of these six impossibility theorems."
  - "Any coherent rescue system for a closed system must have all five grace properties; missing any one creates the specific failure of the corresponding theorem."
  - "Pelagianism (self-salvation) is structurally incoherent, equivalent to a perpetual motion machine, because it violates the theorems that prove self-rescue is impossible."
  - "The coupling coefficient α(u) describes the degree to which a system permits external input; maximum autonomy (u=1) equals zero grace coupling, while surrender of autonomy (u=0) equals maximum coherence restoration."
  - "The six theorems predict that no closed formal system will ever be discovered that validates its own consistency, and no self-contained AI can verify its own alignment without external grounding."
  - "Falsification requires breaking any one of the six theorems—none have been broken in the history of mathematics or physics, and each has been independently strengthened over time."
domains:
  Mathematics: 25
  Physics: 25
  Theology: 30
...
1. **Gödel's Incompleteness Theorems** — No powerful enough math system can prove it's correct from inside itself. You can't check your own work and be sure it's right.
2. **Tarski's Undefinability Theorem** — No math system can define what "true" means using only its own rules. Truth needs a bigger language from outside.
3. **Turing's Halting Problem** — No computer program can predict whether it will ever finish running. A program can't know its own end.
4. **Clausius / Second Law of Thermodynamics** — In a closed system (no energy in or out), disorder always increases. Things fall apart on their own, but they never put themselves back together.
5. **Landauer's Principle** — Every computation costs energy. You can't erase information for free — you have to dump heat somewhere.
6. **Leibniz's Contingency Argument** — Nothing that exists by chance can explain why it exists. You need something outside the chain of causes to ground everything.
