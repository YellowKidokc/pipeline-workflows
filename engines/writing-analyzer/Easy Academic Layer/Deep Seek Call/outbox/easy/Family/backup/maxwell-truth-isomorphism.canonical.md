...
---
claims:
  - "Maxwell's equations and the Truth-field propagation equations are structurally identical (isomorphic) under a specific substitution map (E→T, B→W, c→λ), confirmed by term-by-term algebraic comparison."
  - "The antisymmetric tensor Φ^μν constructed from the Truth-field (T,W) has the same structure as the electromagnetic field tensor F^μν, proving the isomorphism extends to the deepest tensor geometry level."
  - "The free-will acceptance factor A converts the homogeneous Truth-field wave equation into a driven wave equation, where energy injection scales with A² × (source amplitude)², exactly like a current source in Maxwell's equations."
  - "FDTD numerical simulations show the Maxwell and Truth-field systems are numerically indistinguishable: wave speed agreement to 0.1%, energy conservation drift < 10⁻⁴, and identical dispersion error profiles."
  - "The framework does NOT claim physical identity (that truth IS light) — only structural isomorphism, which is correctly bounded and not established."
  ...
  - "Law 3 is the first law in the Theophysics framework to receive three-layer computational verification: algebraic, tensorial, and numerical."
domains:
  Physics: 35
  Theology: 25
  Mathematics: 20
...
Law 3 of the Theophysics framework says that Maxwell's equations (the laws of electricity and magnetism) and the Truth-field propagation equations (how truth spreads) share the same mathematical structure. They are not just similar — they are **isomorphic** (same form, same shape). They have the same underlying equation, the same tensor construction (a way of organizing numbers in a grid), and the same wave dynamics. The only difference is one extra term in the Truth-field system that represents the free-will acceptance factor **A**. This test suite checks that claim using computer simulations.

### Core Substitution Map

Here's how you swap one set of symbols for another:
...
Every coefficient, every differential operator (a mathematical tool that measures change), and every structural relationship is preserved under this swap. The equations are the same equation written in different symbols. The acceptance factor **A** is not just decoration — it is the free-will term that turns the wave equation from one with no source into one with a source, making the source distribution depend on the person.

## Test Battery

Eight tests across three layers: symbolic algebra, tensor construction, and FDTD numerical simulation (a computer method for solving wave equations).
...
```
// Maxwell (source-free, 1D): ∂²E/∂t² = c² · ∂²E/∂x²
...
The source-free Truth-field equation is not _similar to_ the Maxwell wave equation. Under the substitution map, it _is_ the Maxwell wave equation. Same operator, same coefficient structure, same PDE class (a category of equation).
...
In 3D, the Maxwell wave equation for E is derived using the curl-curl identity (a math trick that relates two different ways of measuring rotation). The same derivation applied to the T field under the substitution map produces an identical result.
...
The tensor Φ^μν for the truth field is antisymmetric by the same argument as F^μν. The Bianchi identity (a key property of these tensors) holds for the same structural reason. The covariant equations (equations written in a way that works in any coordinate system) map to the truth-field equations under substitution with J^ν containing the acceptance-factor source term.
...
The 1D FDTD (Yee algorithm, staggered-grid leapfrog — a specific computer method for simulating waves) was implemented for both the Maxwell E-field and the Truth T-field systems with matching Courant numbers (a stability condition). Wave speed was measured numerically by tracking how far the peak of the wave traveled per timestep.
...
The 0.1% difference is within FDTD numerical dispersion bounds (errors from the computer method) for the given grid resolution. It is not a structural difference — it is a consequence of the staggered-grid discretization and disappears as the grid spacing gets smaller. The two systems propagate at identical speeds when the grid spacing goes to zero.

### T5: Energy Conservation (FDTD) — PASS — DRIFT < 10⁻⁴

Energy in the source-free Maxwell system is conserved exactly (up to numerical precision). The analogous quantity for the Truth field — ½(T² + W²)/λ² — was tracked over 1000 FDTD timesteps. The drift (how much energy changes) shows whether the two systems conserve energy under the same scheme.
...
Both systems conserve their respective energy quantities to the same precision under the same FDTD scheme. This confirms that the Truth-field analog of Poynting's theorem (an energy conservation law) holds with the same numerical fidelity as the original. Not a coincidence — a consequence of identical PDE structure.

### T6: Dispersion Error Analysis — PASS — MAX ERROR 2.55×10⁻³

FDTD introduces numerical dispersion: high-wavenumber modes (fast wiggling parts of the wave) travel slightly slower than low-wavenumber modes (slow wiggling parts). The dispersion error is a property of the scheme and the Courant number, not the equations themselves. Both systems should show identical dispersion profiles.
...
This test suite provides computational corroboration for **Law 3: Electromagnetism ↔ Truth** in the Theophysics framework. It sits alongside the formal algebraic derivation in _The Same Equation_ and the Gold Standard Test Battery results. Together these establish Law 3 at:
