...
---
claims:
  - "The lifespan data in Genesis 5 and 11, when plotted against generation number, fits an exponential decay curve with R² = 0.888, proving it is a precise biological dataset, not mythology."
  - "The exponential decay model independently finds an asymptotic floor of 93 years, which matches Psalm 90:10's 70–80 years and the modern global average of 73 years, confirming a real physical limit on human lifespan."
  - "The mechanism behind the exponential decay is 'generational entropy' — a compounding degradation of the information channel between humans and the Logos (God's ordering principle), introduced at the Fall (Genesis 3:1)."
  - "The floor at ~70–93 years is held by the χ vacuum field (the Father's common grace), which maintains a minimum coherence that thermodynamics cannot breach, preventing lifespan from decaying to zero."
  - "The Flood reset some of this accumulated entropy, restarting the exponential decay from a lower baseline (Shem at 600 years) rather than from the pre-Flood plateau (~900 years)."
  - "Moses in Psalm 90:10 identified this floor 3,000 years before modern mathematics, naming it as the Lord's dwelling place across generations."
  - "The convergence of three independent measurements — the Genesis decay model (93), Psalm 90 (70–80), and modern data (73) — points to the same physical reality: a grace-maintained minimum lifespan."
domains:
  Physics: 25
  Theology: 30
  Mathematics: 15
...
# The Curve in the Data: What Genesis Lifespans Actually Show
...
When you plot these lifespans against generation number and fit an exponential decay model (a mathematical formula that describes how something shrinks by a fixed percentage over time), you get this:

$$L(n) = L_0 \cdot e^{-n/\tau} + L_{\text{floor}}$$

This equation means: the lifespan at any generation (L(n)) equals the starting lifespan (L₀) multiplied by a shrinking factor (e^{-n/τ}), plus a minimum floor (L_floor) that it can't drop below.

L(n) Lifespan at generation *n* — how long people lived

L₀ Pre-decay baseline — the starting point before the decay

e^{-n/τ} Exponential decay — compounding entropy across generations

τ Decay timescale — ~3–4 generations for lifespan to drop to ~37% of L₀

L_floor The asymptotic floor — **the minimum lifespan that entropy cannot breach**
...
*(Imagine a graph here: the x-axis is "Generation from Adam" (0 to 22), the y-axis is "Lifespan in years" (0 to 1000). Pre-Flood points (generations 0-9) are all around 900-969. A vertical line marks "THE FLOOD" between generation 9 and 10. Post-Flood points (generations 10-22) drop steeply from 600 down to 110. A smooth curved line (the decay fit) follows these points. A horizontal dashed line near the bottom is labeled "L_floor = 93".)*

**Goodness of Fit**

R² = 0.888

This means the model explains 88.8% of the variation in the data — a very strong fit.
...
Entropy (the natural tendency toward disorder) accumulates this way. The framework calls it *generational entropy* — the accumulated misalignment with the Logos (God's ordering principle, like a radio signal through static) that compounds across generations, each generation inheriting the decoherence (signal degradation) of all prior generations plus their own. The first generation after the Fall has Adam's entropy. The second has Adam's plus Seth's. And so on. The compounding is the mechanism. The exponential is the signature.
...
The Father as quantum vacuum (the basic energy state of empty space) maintains the irreducible zero-point energy that cannot be evacuated from any region of space. The grace floor — the common grace that God does not withdraw even from the most degraded system — maintains a minimum coherence that thermodynamics cannot breach.
