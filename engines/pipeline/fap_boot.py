...
def create_engine(pg_dsn: str = None) -> PipelineEngine:
    ...
    engine.register_station(FrameworkClassifierStation(input_dir=DIRS["lossless"], output_dir=DIRS["graded"], review_dir=os.path.join(DIRS["review"], "framework-classifier"), fail_dir=os.path.join(DIRS["rejected"], "framework-classifier")), pipeline_name="paper-mill", order=5)
    engine.register_station(PaperGraderStation(input_dir=DIRS["vectorized"], output_dir=DIRS["graded"], review_dir=os.path.join(DIRS["review"], "paper-grader"), fail_dir=os.path.join(DIRS["rejected"], "paper-grader")), pipeline_name="paper-mill", order=8)
    engine.register_station(AxiomMapperStation(input_dir=DIRS["graded"], output_dir=DIRS["axiom_mapped"], review_dir=os.path.join(DIRS["review"], "axiom-mapper"), fail_dir=os.path.join(DIRS["rejected"], "axiom-mapper")), pipeline_name="paper-mill", order=7)
